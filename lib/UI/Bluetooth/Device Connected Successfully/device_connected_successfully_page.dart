...
class DeviceConnectedSuccessfullyPage extends StatefulWidget {
  ...
  const DeviceConnectedSuccessfullyPage(
      {Key? key, required this.cubit, required this.device})
      : super(key: key);

  @override
  State<DeviceConnectedSuccessfullyPage> createState() =>
      _DeviceConnectedSuccessfullyPageState();
}

class _DeviceConnectedSuccessfullyPageState
    extends State<DeviceConnectedSuccessfullyPage> {
                        ...
                        children: [
                          ...
                          BlocBuilder(
                              ...
                              builder: (context, state) {
                                if (state is DeviceConnectedSuccessfullyState) {
                                  return bodyText(
                                      '${widget.device.platformName ?? ''} has been connected\nsuccessfully');
                                }
                                return Container(); // Or any other widget you want as a fallback
                              }),
                          SizedBox(
                            height: 182.h,
                          ),
