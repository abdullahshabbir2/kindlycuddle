...
class HomeCubit extends Cubit<HomeState> {
  ...
  HomeCubit(this.initialParams, this.navigator, this.getPulseWeightUseCase,
      this.setPulseWeightUseCase, this.store, this.profileDataUseCase)
      : super(HomeState.initial(initialParams: initialParams));

  void onInit(HomeInitialParams initialParams) => emit(state.copyWith());

  void getPulseWeight() {
    getPulseWeightUseCase.execute().then((value) => value.fold((l) {}, (r) {
  ...
  void setPulseWeight(double pulse, double weight) {
    emit(
        state.copyWith(pulse: pulse, weight: weight, showBlurredVision: false));
    setPulseWeightUseCase
        .execute(pulse, weight)
        .then((value) => value.fold((l) {
              ToastMessage()
                  .showMessage(l.error, ColorsConstants.failureToastColor);
            }, (r) {
              ToastMessage()
                  .showMessage('Data set', ColorsConstants.successToastColor);
            }));
  }
  ...
  getProfileData() {
    ...
    debugPrint(id);
    profileDataUseCase.getProfileData(id).then((value) => value.fold((l) {
          ToastMessage()
              .showMessage(l.error, ColorsConstants.failureToastColor);
        }, (r) {
